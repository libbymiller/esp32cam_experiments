...
//compare the same pixel in 2 frames
 ...
 //if the change is greter than the delta in either direction
  ...
  ////          resultFrame->buf[ i ] = (uint8_t) 255;//???
            resultFrame->buf[ i ] = (uint8_t) 80;//???tmp
...
//cogx is scaled to width of frame, so we gte range 0-1
        m_attentionX = ((float)cogX  / fb->width);
        m_attentionY = 1 - ((float)cogY  / fb->width); // use the larger dimension so x & y get the same scaling

//colour in some pixels
//how do we get back to the buffer localtions from the cogx and cogy?
//point = width * y-1 +  x (depending on where zero, zero is)
//could use fmt2rgb888 for color maybe?
//here making a cross
        int point = (fb->width * cogY)+cogX;
        resultFrame->buf[ point ] = (uint8_t) 255;// ideally woudl be red
        resultFrame->buf[ point+1 ] = (uint8_t) 255;
        resultFrame->buf[ point+2 ] = (uint8_t) 255;
        resultFrame->buf[ point+3 ] = (uint8_t) 255;
        
        resultFrame->buf[ point-1 ] = (uint8_t) 255;
        resultFrame->buf[ point-2 ] = (uint8_t) 255;
        resultFrame->buf[ point-3 ] = (uint8_t) 255;

        resultFrame->buf[ point + (fb->width) ] = (uint8_t) 255;
        resultFrame->buf[ point + (fb->width) * 2 ] = (uint8_t) 255;
        resultFrame->buf[ point + (fb->width) * 3 ] = (uint8_t) 255;
